...
{
public:
    MarchingCubes(const Vec3i &dimensions, const Vec3f &size, const Vec3f &origin);
    // MarchingCubes(const int &dim_x, const int &dim_y, const int &dim_z, const float &size, std::vector<float> &origin);
    ~MarchingCubes();
