...
def load_pose(file, matrix=True):
    mode = file[-3:]    
    if mode == 'txt':
        traj = load_pose_txt(file, matrix)
    elif mode == 'npy':
        traj = np.load(file)
...
def load_pose_txt(filename, matrix=True):
    """
    ...
    list = [[line.split(" ")[0]]+[float(v) for v in line.split(" ")[1:] if v.strip()!=""] for line in lines if len(line)>0 and line[0]!="#"]
    list_ok = []
...
def transform44(l):
    ...
    """
    t = l[1:4]
    q = np.array([l[5], l[6], l[7], l[4]], dtype=np.float64, copy=True)
    # t = np.array(l[5:8], dtype=np.float64, copy=True)
    # q = np.array(l[1:5], dtype=np.float64, copy=True)
    nq = np.dot(q, q)
    if nq < _EPS:
        return np.array([
        [                1.0,                 0.0,                 0.0, t[0]],
...
def Quaternion(pose):
    w = rot_to_quat(pose[:3,:3]).to(pose.device)
    return torch.hstack([pose[:3,3], w])